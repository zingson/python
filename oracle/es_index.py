...
# 数据库连接配置 user/pass@host:port/orcl
db_config = 'unionlive/unionlive@proxy.unionlive.com:1521/orcl'
# SQL查询语句
db_sql = 'SELECT * from ULTAB_SYS_USER'
# ES服务地址与索引名称
...
for d in dataList:
    ...
    res = requests.put('%s/%s' % (es_index, d[es_index_id]), proxies=proxies, headers=headers, data=json.dumps(d))
